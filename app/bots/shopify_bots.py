...
def create_shopify_order(url="https://ns8-internal-test-store.myshopify.com/", headless=False):
    ...
    st_ad = browser.find_element_by_xpath(
    ...
    state_picker = browser.find_element_by_xpath(
    ...
    zip = browser.find_element_by_xpath(
    ...
    for i in range(len(iframes)):
        ...
        logging.info('iframe title: {}'.format(
            iframes[i].get_attribute('name')))
    ...
    if len(iframes) > 2:
        ...
        element = browser.find_element_by_xpath(
            '/html/body/div[1]/div/div/main/div[1]/div/form/div[3]/div[1]/button/span')
        browser.execute_script("arguments[0].click();", element)
        ...
        code = browser.find_element_by_id(
            'checkout_credit_card_verification_value')
        code.send_keys(fake.credit_card_security_code()[0:3])
        time.sleep(2)
...
def create_shopify_order_kacoko(url="https://kacoko.myshopify.com/", headless=False):
    ...
    # go to all products:
    product_page = '/html/body/div[2]/div[2]/header/div/nav/ul/li[2]/a/span'
    browser.find_element_by_xpath(product_page).click()

    # Choose Products

    # get list of all products:
    prod = browser.find_elements_by_class_name('product-card')
    ...
    # Add product to cart
    browser.find_element_by_xpath(
        '/html/body/div[3]/main/div[1]/div/div/div[2]/div[1]/form/div[3]/div/button/span[1]').click()
    ...
    # Go to the cart
    browser.find_element_by_xpath(
        '/html/body/div[1]/div/a').click()

    # Wait again
    time.sleep(5)

    # Adjust number of items to add to cart
    # qty = browser.find_element_by_xpath('//*[@id="updates_large_32090984448054:0a3df124a3a0a7e391927ae1d53b1810"]')[0]
    # qty.send_keys(Keys.BACKSPACE)
    # qty.send_keys(Keys.DELETE)
    # qty.send_keys(str(random.choice(range(1, 11))))
    ...
    # Procede to checkout
    browser.find_element_by_xpath(
        '/html/body/div[3]/main/div/div/div[1]/form/div/div/div/div[5]/div[1]/input').click()
